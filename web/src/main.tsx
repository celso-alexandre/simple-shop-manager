...
const errors: { [key in MainExceptionKeys]: string } = {
  Provider_UQ_document_conflict:
    'Fornecedor já cadastrado (documento já utilizado)',
  ...
  Sale_SaleItem_zero_length_badRequest:
    'Não é permitido gravar uma venda sem itens',
  SalePaidCost_zero_paidValue_badRequest:
    'Não há nenhum valor a ser pago para a seleção',
};
function Main() {
  ...
  const errorLink = onError(
    ({ graphQLErrors, networkError, forward, operation }) => {
      let clipboard = '';
      if (graphQLErrors) {
        clipboard += graphQLErrors.map(({ message, locations, path }) => {
      ...
      graphQLErrors?.forEach((e) => {
        ...
        const httpStatus =
          parseInt((e as any).extensions?.response?.statusCode, 10) ||
          undefined;
